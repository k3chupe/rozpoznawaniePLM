...
model = tf.keras.models.load_model(os.path.join(BASE_DIR, "model_gesty_ruchome.keras"))
with open(os.path.join(BASE_DIR, "etykiety_ruch.pkl"), "rb") as f:
    lb = pickle.load(f)
    ...
    while True:
        ...
        if wynik.multi_hand_landmarks:
            for hand_landmarks in wynik.multi_hand_landmarks:
                ...
                cechy = cechy_dynamiczne_kat(hand_landmarks)
                sekwencja_klatek.append(cechy)
        ...
        for i, gest_z_historii in enumerate(reversed(historia_gestow)):
            kolor = (0, 255, 255) if i == 0 else (150, 150, 150)
